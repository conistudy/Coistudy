...
const initCharts = () => {
	const city = store.list.areaTree[2];
	console.log(city);

	store.item = city;
	const data = city.children.map((v: any) => {
		return {
			name: v.name,
...
const initLine = () => {
	...
	charts.setOption({
		...
		xAxis: {
			type: "category",
			data: store.cityDetail.map((v: any) => v.name),
			axisLine: {
				lineStyle: {
					color: "#fff",
				},
		...
		series: [
			{
				data: store.cityDetail.map((v: any) => v.today.confirm),
				type: "line",
				smooth: true,
			},
		],
